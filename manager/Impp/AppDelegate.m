...
//    [[AnPush shared] register:@[ @"_IMPP_DEFAULT_" ] overwrite:YES success:^(NSString *anid) {
//        NSLog(@"AnPush register successful, anid: %@", anid);
//        [[[HXIMManager manager]anIM] bindAnPushService:anid appKey:LIGHTSPEED_APP_KEY clientId:[[HXIMManager manager] clientId]  success:^{
//            NSLog(@"AnIM bindAnPushService successful");
//        } failure:^(ArrownockException *exception) {
//            NSLog(@"AnIm bindAnPushService failed, error : %@", exception.getMessage);
//        }];
//    } failure:^(ArrownockException *exception) {
//        NSLog(@"AnPush failed to register, error: %@", exception.getMessage);
//    }];
